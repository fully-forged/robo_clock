defmodule RoboClock.PubSub do
  ...
  def child_spec(_opts) do
    Registry.child_spec(keys: :duplicate, name: RoboClock.PubSub)
  end

  @doc """
  Subscribes a process to a topic. The process will receive messages
  in the shape of `{RoboClock.PubSub.Broadcast, topic, message}`.
  """
  ...
  def publish(topic, message) do
    Registry.dispatch(__MODULE__, topic, fn entries ->
      for {pid, _} <- entries, do: send(pid, {RoboClock.PubSub.Broadcast, topic, message})
    end)
  end
end
